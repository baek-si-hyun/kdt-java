...
//			session에 있는 회원 정보로 마이페이지에 필요한 정보를 조회한다.
			...
			memberVO.setRecommanderId(resultSet.getString(6));
...
//	추천수
	public int getCountOfRecommander(String memberId) {
		String query = "SELECT COUNT(ID) FROM TBL_MEMBER WHERE RECOMMENDER_ID = ?";
		int recommanderCount = 0;
		try {
			connection = DBConnecter.getConnection();
			preparedStatement = connection.prepareStatement(query);
			preparedStatement.setString(1, memberId);
			...
			resultSet.next();
			recommanderCount = resultSet.getInt(1);
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
			try {
		...
		return recommanderCount;
		
	}
	
	
//	나를 추천한 사람
	public List<MemberVO> getRecommenders(){
